...
HPA scales API pods based on CPU metrics from the Kubernetes Metrics Server.
...
- `TransactionCreatedConsumer`: 50 concurrent messages per pod
- `TransactionCancelledConsumer`: 25 concurrent messages per pod
...
The Reporting API uses a Polly circuit breaker on Redis to protect against cache outages:
...
When the circuit is open, queries fall through to PostgreSQL directly. The cache is repopulated after the break period ends.
...
| Component | Strategy |
|---|---|
| PostgreSQL writes | Single primary — vertical scale first |
| PostgreSQL reads | Same instance (reporting schema is read-only) |
| Redis | Single node in dev; Redis Cluster in production |
| RabbitMQ | Single node in dev; mirrored queues across 3 nodes in production |
