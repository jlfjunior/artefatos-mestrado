...
# Production (requires manual approval gate in CI)
helm upgrade --install financial-balance ./helm/financial-balance \
...
|---|---|---|
| `GET /health/live` | Liveness — is the process running? | Always 200 (process is up) |
| `GET /health/ready` | Readiness — can it serve traffic? | PostgreSQL + Redis + RabbitMQ all reachable |
...
{
    Predicate = _ => false   // liveness: always 200, just confirms process is alive
});

app.MapHealthChecks("/health/ready", new HealthCheckOptions
{
    Predicate = check => check.Tags.Contains("ready")
    // readiness: confirms Postgres + Redis + RabbitMQ connections
});
```

### Docker Compose (Local)

Both API services expose their `/health/ready` endpoint as a Docker healthcheck. The `start_period` gives the app time to complete EF Core migrations before the first check fires:
...
The Reporting Worker has no HTTP server and therefore no Docker healthcheck. Monitor it via `docker compose logs -f reporting-worker`.

### Kubernetes Probes

The Kubernetes deployment configures separate liveness and readiness probes on port 8080:
...
The readiness probe gates traffic routing — a pod is only added to the Service endpoint list once `/health/ready` returns 200. The liveness probe restarts the container if the process becomes unresponsive.
...
|---|---|---|
| Metrics | Prometheus via OpenTelemetry | `/metrics` |
| Health (liveness) | ASP.NET Core health checks | `/health/live` |
| Health (readiness) | ASP.NET Core health checks + dependencies | `/health/ready` |
| Traces | OpenTelemetry → Jaeger | gRPC exporter |
| Logs | Serilog → console (structured JSON) | stdout |
