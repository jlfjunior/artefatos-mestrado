...
using FinancialBalance.ReportingInfrastructure;
...
// MediatR + Validation pipeline (application layer assembly)
...
// Reporting infrastructure (EF Core, Redis, MassTransit consumers, rollup job)
...
// Rate limiting — 2000/min with sliding window to handle report bursts
builder.Services.AddRateLimiter(options =>
{
    options.AddSlidingWindowLimiter("reports", limiter =>
    {
...
builder.Services.AddOpenTelemetry()
    .ConfigureResource(r => r.AddService("financial-balance-reporting-api"))
...
{
    c.SwaggerDoc("v1", new() { Title = "Financial Balance — Reporting API", Version = "v1" });
