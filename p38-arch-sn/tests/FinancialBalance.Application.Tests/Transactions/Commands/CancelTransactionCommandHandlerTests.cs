...
public class CancelTransactionCommandHandlerTests
{
    ...
    public async Task Handle_ValidCommand_CancelsTransaction()
    {
        ...
        _repository.GetByIdWithTransactionAsync(account.Id, tx.Id, Arg.Any<CancellationToken>()).Returns(account);
    ...
    {
        _repository.GetByIdWithTransactionAsync(Arg.Any<Guid>(), Arg.Any<Guid>(), Arg.Any<CancellationToken>()).Returns((Account?)null);
    ...
    {
        var account = Account.Create("Test", "T-001", AccountType.Checking, Currency.BRL);
        _repository.GetByIdWithTransactionAsync(account.Id, Arg.Any<Guid>(), Arg.Any<CancellationToken>()).Returns(account);
