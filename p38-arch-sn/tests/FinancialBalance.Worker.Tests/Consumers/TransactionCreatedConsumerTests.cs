...
public class TransactionCreatedConsumerTests
{
    ...
    public async Task Consume_WhenNoDailySummaryExists_CreatesAndUpserts()
    {
        ...
        var msg = new TransactionCreated(Guid.NewGuid(), AccountId, TransactionType.Incoming, 1000m, TxDate, TransactionCategory.Revenue);
    ...
    public async Task Consume_WhenDailySummaryExists_AppliesTransactionToExisting()
    {
        ...
        existing.ApplyTransaction("Incoming", 500m, "Revenue");
        _dailyRepo.GetAsync(AccountId, Arg.Any<DateOnly>(), Arg.Any<CancellationToken>()).Returns(existing);

        var msg = new TransactionCreated(Guid.NewGuid(), AccountId, TransactionType.Incoming, 300m, TxDate, TransactionCategory.Revenue);
    ...
    public async Task Consume_InvalidatesDailyCacheKey()
    {
        ...
        var msg = new TransactionCreated(Guid.NewGuid(), AccountId, TransactionType.Incoming, 100m, TxDate, TransactionCategory.Revenue);
    ...
    public async Task Consume_InvalidatesMonthlyCacheKey()
    {
        ...
        var msg = new TransactionCreated(Guid.NewGuid(), AccountId, TransactionType.Outgoing, 200m, TxDate, TransactionCategory.Supplier);
    ...
    public async Task Consume_OutgoingTransaction_UpdatesOutgoingTotal()
    {
        ...
        var msg = new TransactionCreated(Guid.NewGuid(), AccountId, TransactionType.Outgoing, 450m, TxDate, TransactionCategory.Supplier);
